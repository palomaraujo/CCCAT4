...
test("Deve criar um pedido zerado com CPF válido", function(){
...
test("Deve tentar criar um pedido zerado com CPF inválido", function(){
...
test("Deve criar um pedido com 3 itens com cupom de desconto expirado", function(){
    ...
    order.addItem(new Item(2, "Vídeo", "DVD", 50), 1);
    order.addItem(new Item(3, "Vídeo", "VHS", 10), 2);
    order.addCoupon(new Coupon("VALE20", 20, new Date("2021-12-08")));
    const total = order.getTotal();
    expect(total).toBe(160);
})

test("Deve criar um pedido com 3 itens com o calculo do frete com a estratégia default", function(){
    const cpf = '839.435.452-10';
