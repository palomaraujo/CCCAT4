...
export default class SimulateFreight{
  constructor(readonly itemRepository: ItemRepository, readonly freighCalculator: FreightCalculator){
  ...
  async execute(input:SimulateFreightInput):Promise<SimulateFreightOutput> {
    ...
    for(const inputItem of input.items){
      ...
      amount += this.freighCalculator.calculate(item) * inputItem.quantity
    }
